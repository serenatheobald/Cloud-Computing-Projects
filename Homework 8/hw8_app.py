...
zone_request_counter = Counter()


# Initialize the Google Cloud Logging client
client = LoggingClient(project = "ds-561-first-project")
logger = client.logger('homework8_logger')

# Initialize loggers for your VMs
logger_a = client.logger('hw8-central1-a')
logger_b = client.logger('hw8-central1-b')
...
def get_zone():
    try:
        ...
        if response.status_code == 200:
            ...
            logger.log_text(f"Zone retrieved successfully: {zone}", severity='INFO')
            return zone
        else:
            logger.log_text("Failed to retrieve zone: Metadata server returned non-200 response", severity='WARNING')
            return "Unknown Zone"
    except Exception as e:
        logger.log_text(f"Failed to get zone: {e}", severity='ERROR')
...
def serve_file(bucket,dir_name, dir2_name, file_name):
    ...
    if content:
        ...
        # Log zone information for VM A if it's in us-central1-a
        if zone == 'us-central1-a':
            logger_a.log_text(f"Zone retrieved for VM A: {zone}", severity='INFO')
        
        # Log zone information for VM B if it's in us-central1-b
        elif zone == 'us-central1-b':
            logger_b.log_text(f"Zone retrieved for VM B: {zone}", severity='INFO')
                
        
        logger.log_text(f"Zone retrieved: {zone}", severity='INFO')
